...
export default function CaseOwnerProfilePage() {
  ...
  const caseOwner = searchParams.get('caseOwner')
    ...
    (async () => {
      ...
      try {
        ...
        if (!res.ok || !data?.success) {
          if (res.status === 403) {
            setError("Access denied. Only police users can view case owner profiles.")
          } else if (res.status === 404) {
            setError("Case owner profile not found.")
          } else {
  ...
  }, [getToken, isLoaded, isSignedIn, caseOwner])
  ...
  const renderGeneralUser = () => {
    ...
    const dobStr = profile?.dateOfBirth ? format(new Date(profile.dateOfBirth), "PPP") : "—"
    return (
        ...
        <div className="lg:col-span-5 w-full lg:sticky lg:top-6">
          <div className="rounded-2xl border border-border bg-card px-6 py-6 shadow-sm">
            ...
            {!loading && !error && profile && (
              <div className={`${mobileDetailsOpen ? "mt-3" : ""} ${mobileDetailsOpen ? "" : "hidden lg:block"}`}>
                <div className="space-y-4">
                  {profile?.role === "general_user" && renderGeneralUser()}
                  {profile?.role === "police" && renderPolice()}
                  {profile?.role === "NGO" && renderNGO()}
                </div>
              </div>
            )}
          </div>
